...
class TestSinusoidalPoissonGenerator(unittest.TestCase):
    ...
    def test_sample_network_sinusoidal_poisson_rate_curr_same_spikes(self):
        """
            Verify that the network can be created and ran even if all neurons
            receive the same background stimulus.
        """
        np.random.seed(424242)
        ...
        sampler_config.source_config = self.get_source_config()
        sampler_config.source_config.individual_spike_trains = False
